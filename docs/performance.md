...
The 2048×1024 Milky Way panorama adds 284 KiB to the transfer and approximately 10.7 MiB of GPU memory including mip levels. It replaces the previous CPU-generated sky without adding a render pass or per-frame upload. On physical GPUs its sampler uses up to `8:1` hardware anisotropy: the driver adds footprint samples only for the strongly elongated derivatives produced by lensing, while ordinary sky pixels retain the normal mipmapped path. Software renderers retain ordinary mip filtering rather than emulating this optional hardware path.
